...
class MockAIProvider {
    name = 'Prototype Semantic AI Engine (Offline Heuristic)';
    isConfigured = true;
    async analyze(input) {
        ...
        const hasAuthorityImpersonation = /(sbi|hdfc|icici|axis|punjab national|pnb|reserve bank|rbi|income tax|telecom|airtel|jio|amazon support|google pay support|paytm care|customs officer|police cyber cell|electricity board|bescom|mpeb)/i.test(text);
        const hasUrgencyOrCoercion = /(immediately|urgent|today itself|within 24 hours|account will be blocked|account suspended|deactivated|disconnected|power will be cut|legal action|arrest warrant|last warning)/i.test(text);
        const hasKycPretext = /(kyc|pan update|aadhaar link|identity verification|re-kyc|document upload|account closure notice)/i.test(text);
        const hasRefundPretext = /(refund|cashback|overpayment|money return|credit your account|claim refund|pending reimbursement)/i.test(text);
        const hasPrizeOrLottery = /(congratulations|won|lottery|kbc|lucky draw|cash prize|gift voucher|selected for prize)/i.test(text);
        const hasRemoteAccess = /(anydesk|teamviewer|rustdesk|quicksupport|screen share|install this app to fix)/i.test(text);
        const hasPinRequest = /(enter.*pin|upi pin to receive|enter upi pin for credit|share otp|forward this sms)/i.test(text);
        const hasJobOrTask = /(part time job|telegram task|earn 5000 per day|like youtube videos|crypto rating task|rating hotel)/i.test(text);
        const hasSuspiciousUrlPattern = /(http:\/\/|\.xyz|\.top|\.live|\.tk|\.cc|bit\.ly|tinyurl|is\.gd|cutt\.ly|firebaseapp\.com|pages\.dev)/i.test(text);
        const hasLegitimateBankReceiptPattern = /(debited by|credited to your|available balance|acct.*ending|inb\/|txn#|ref no|utr no)/i.test(text) &&
            !/(click here|verify at|call helpline 9[0-9]{9}|account will be blocked)/i.test(text);
        const hasLegitimateOrderPattern = /(delivered|arriving by|out for delivery|tracking id|order placed|swiggy|zomato|amazon order|flipkart)/i.test(text) &&
        ...
        if (hasRemoteAccess) {
            ...
            recommendations.push('Never install AnyDesk, TeamViewer, or QuickSupport on instructions from an incoming caller.', 'If already installed, turn off Wi-Fi/Mobile Data immediately and uninstall the application.', 'Contact your bank immediately to freeze your internet banking and UPI accounts.');
        }
        // Scenario 2: Refund QR Scam / Collect Request Scam
            ...
            recommendations.push('NEVER enter your UPI PIN to receive money or refunds. PIN entry always debits your account.', 'Decline the UPI collect request in your payment app immediately.', 'Only seek refunds via the official app where the purchase was made.');
        }
        // Scenario 3: Fake KYC / Account Suspension Phishing
            ...
            recommendations.push('Do not click the link or provide Aadhaar, PAN, debit card numbers, or OTPs.', 'Official KYC is conducted in-person at bank branches or via verified video KYC in official bank apps.', 'Forward the suspicious message to the Indian Government Citizen Cyber Fraud reporting number 1930.');
        }
        // Scenario 4: Lottery / Prize / Cashback Scam
            ...
            recommendations.push('Never send advance money to claim a prize or lottery.', 'Legitimate lotteries deduct applicable tax at source before distributing winnings.', 'Block and report the contact immediately.');
        }
        // Scenario 5: Customer Support Impersonation
            ...
            recommendations.push('Never search for customer care numbers on Google Maps or social media—scammers plant fake numbers.', 'Always obtain customer support contact details directly from inside your official mobile application.');
        }
        // Scenario 6: Job / Task Scam
            ...
            recommendations.push('Legitimate companies never ask employees to pay money to receive work or salary.', 'Do not join Telegram groups claiming automated investment returns.');
        }
        // Scenario 7: Standalone Suspicious URL
            ...
            recommendations.push('Do not open the link or enter banking information.', 'Inspect the root domain carefully before interacting with any payment portal.');
        }
        // Scenario 8: Legitimate Transaction / Notification
            ...
            recommendations.push('Transaction appears legitimate. Verify against your official banking transaction passbook.');
        }
