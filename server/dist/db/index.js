...
exports.clearAnalyses = clearAnalyses;
...
const dbPath = path_1.default.join(dbDir, 'sentinel.db');
...
const count = exports.db.prepare('SELECT COUNT(*) as count FROM analyses').get().count;
if (count === 0) {
    const seedRecords = [
        {
            id: 'sentinel-seed-01',
  ...
  `);
    const insertMany = exports.db.transaction((records) => {
...
function saveAnalysis(analysis, inputType, inputPreview) {
...
function getAnalyses(options) {
    ...
    const total = exports.db.prepare(countQuery).get(params).total;
    const dataQuery = `
  ...
  `;
    const rows = exports.db.prepare(dataQuery).all({ ...params, limit, offset });
...
function getAnalysisById(id) {
    const r = exports.db.prepare('SELECT * FROM analyses WHERE id = ?').get(id);
    if (!r)
        return null;
    return {
        id: r.id,
...
function deleteAnalysis(id) {
    const res = exports.db.prepare('DELETE FROM analyses WHERE id = ?').run(id);
    return res.changes > 0;
}
function clearAnalyses() {
    exports.db.prepare('DELETE FROM analyses').run();
    return true;
}
function getStats() {
    const total = exports.db.prepare('SELECT COUNT(*) as count FROM analyses').get().count;
    const highRisk = exports.db.prepare("SELECT COUNT(*) as count FROM analyses WHERE classification = 'HIGH_RISK'").get().count;
    const suspicious = exports.db.prepare("SELECT COUNT(*) as count FROM analyses WHERE classification = 'SUSPICIOUS'").get().count;
    const safe = exports.db.prepare("SELECT COUNT(*) as count FROM analyses WHERE classification = 'SAFE'").get().count;
    // Breakdown by threat type
    const threatRows = exports.db.prepare(`
  ...
  `).all();
    ...
    const recentAnalyses = exports.db.prepare(`
