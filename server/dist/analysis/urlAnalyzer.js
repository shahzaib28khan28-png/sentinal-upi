...
exports.analyzeUrlDeterministically = analyzeUrlDeterministically;
...
function analyzeUrlDeterministically(rawUrl) {
    ...
    const suspiciousKeywordsFound = SUSPICIOUS_URL_KEYWORDS.filter((kw) => hostname.includes(kw) || pathname.includes(kw));
    ...
    for (const [brandName, pattern] of Object.entries(BRAND_PATTERNS)) {
        if (pattern.regex.test(hostname)) {
            const isOfficial = pattern.officialDomains.some((od) => hostname === od || hostname.endsWith('.' + od));
            if (!isOfficial) {
