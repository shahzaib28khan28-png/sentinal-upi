...
import { Indicator, ParsedUpiDetails } from '../types';

export function analyzeQrDeterministically(qrData: string, userContext?: string): {
  upiDetails: ParsedUpiDetails;
  indicators: Indicator[];
  deterministicRisk: number;
  warnings: string[];
  mandatorySafetyNotice: string;
} {
  const upiDetails = parseUpiString(qrData);
  const indicators: Indicator[] = [];
  const warnings: string[] = [...upiDetails.warnings];
  let deterministicRisk = 10;

  const mandatorySafetyNotice =
    'CRITICAL UPI SAFETY RULE: Scanning a QR code does not receive money automatically. NEVER enter your UPI PIN to receive money or refunds. UPI PIN is exclusively used for sending money from your account.';
