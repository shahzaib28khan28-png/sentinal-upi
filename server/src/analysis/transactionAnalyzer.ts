import { TransactionAnalysisInput, Indicator } from '../types';
import { parseUpiString } from './upiParser';

export function analyzeTransactionDeterministically(input: TransactionAnalysisInput): {
  ...
  const upiDetails = parseUpiString(input.receiverUpi);
