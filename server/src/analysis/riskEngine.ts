...
export function synthesizeRiskAnalysis(params: CombineRiskParams): AnalysisResult {
  const {
    inputType,
    aiResult,
  ...
  const mergedIndicators: Indicator[] = [];
  const seenIndicatorNames = new Set<string>();
  ...
  const hasCriticalDeterministic = deterministicIndicators.some(
    (i) => i.severity === 'CRITICAL'
  );
  const hasHighDeterministic = deterministicIndicators.some(
    (i) => i.severity === 'HIGH'
  );
  ...
  let threatType: ThreatType = aiResult.threatType;
  ...
  for (const r of aiResult.recommendations || []) {
    if (r && r.trim()) allRecs.add(r.trim());
  }

  // Deduplicate social engineering techniques
  const socialTechniques = Array.from(
    new Set(aiResult.socialEngineeringTechniques || [])
  );
