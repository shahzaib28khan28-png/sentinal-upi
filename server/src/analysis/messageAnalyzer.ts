import { MessageAnalysisInput, Indicator, UrlSecurityDetails, ParsedUpiDetails } from '../types';
import { analyzeUrlDeterministically } from './urlAnalyzer';
import { parseUpiString } from './upiParser';

export function analyzeMessageDeterministically(input: MessageAnalysisInput): {
  ...
  const extractedUrls: UrlSecurityDetails[] = [];
  const extractedUpis: ParsedUpiDetails[] = [];
  ...
  for (const rawMatch of matches) {
    ...
    const urlAnalysis = analyzeUrlDeterministically(cleanUrl);
  ...
  for (const rawUpi of upiMatches) {
    const parsed = parseUpiString(rawUpi);
